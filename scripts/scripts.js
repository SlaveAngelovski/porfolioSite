...
function toggleClasses(el, classesToToggle, method) {
...
function elObserverCallback(entries, observer, classesToToggle) {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      entry.target.classList.add(...classesToToggle);
    } else {
      entry.target.classList.remove(...classesToToggle);
    }
  });
}
